...
local RENDER_VARIANTS = {
    ...
    {
        'scircle_left', 'admin', 'title', 'unseen_output', 'padding',
        'scircle_right'
    }, { 'scircle_left', 'wsl', 'title', 'padding', 'scircle_right' },
    {
        'scircle_left', 'wsl', 'title', 'unseen_output', 'padding',
        'scircle_right'
    }
}
...
-- stylua: ignore
local colors = {
    text_default = { bg = '#45475A', fg = '#eff1f5' },
    text_hover = { bg = '#5D87A3', fg = '#45475A' },
    text_active = { bg = '#eff1f5', fg = '#45475A' },

    unseen_output_default = { bg = '#45475A', fg = '#FFA066' },
    unseen_output_hover = { bg = '#5D87A3', fg = '#FFA066' },
    unseen_output_active = { bg = '#eff1f5', fg = '#FFA066' },

    scircle_default = { bg = 'rgba(204, 208, 218, 1)', fg = '#45475A' },
    scircle_hover = { bg = 'rgba(204, 208, 218, 1)', fg = '#5D87A3' },
    scircle_active = { bg = 'rgba(204, 208, 218, 1)', fg = '#eff1f5' }
}
...
function Tab:set_info(event_opts, tab, max_width)
    local process_name = clean_process_name(tab.active_pane
        .foreground_process_name)

    self.is_wsl = process_name:match('^wsl') ~= nil
    self.is_admin = (tab.active_pane.title:match('^Administrator: ') or
        tab.active_pane.title:match('(Admin)')) ~= nil
    self.unseen_output = false
    self.unseen_output_count = 0

    if not event_opts.hide_active_tab_unseen or not tab.is_active then
        self.unseen_output, self.unseen_output_count =
            check_unseen_output(tab.panes)
    end

    local inset = (self.is_admin or self.is_wsl) and TITLE_INSET.ICON or
        TITLE_INSET.DEFAULT
    ...
    self.title = create_title(process_name, tab.active_pane.title, max_width,
        inset)
end

function Tab:create_cells()
    local attr = self.cells.attr
    self.cells:add_segment('scircle_left', GLYPH_SCIRCLE_LEFT):add_segment(
        'admin', ' ' .. GLYPH_ADMIN):add_segment('wsl', ' ' .. GLYPH_LINUX)
        :add_segment('title', ' ', nil, attr(attr.intensity('Bold')))
        :add_segment('unseen_output', ' ' .. GLYPH_CIRCLE):add_segment(
        'padding', ' '):add_segment('scircle_right', GLYPH_SCIRCLE_RIGHT)
end
...
function Tab:update_cells(event_opts, is_active, hover)
    ...
    self.cells:update_segment_colors('scircle_left',
        colors['scircle_' .. tab_state])
        ...
        :update_segment_colors('unseen_output',
            colors['unseen_output_' .. tab_state])
...
function Tab:render()
    ...
    if self.is_wsl then variant_idx = 5 end
